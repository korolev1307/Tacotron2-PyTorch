...
short_form_map = {
    'м-е': 'месторождение', 'куб.': 'кубических',
    'млрд.': 'миллиардов', 'м.': 'метров', 'м3': 'метров кубических',
    'сут.': 'сутки', 'млн.': 'миллионов', 'м3.': 'метров кубических',   
}
...
def short_form_to_reading_form(text):
    for key, value in short_form_map.items():
...
def replace_multiples_spaces(text):
    text = re.sub(' +', ' ', text)
    return text


def delete_spec_symbols(text):
    text = re.sub('[\"\'()«»]', "", text)
    return text


def normalize_russian(text):
    text = short_form_to_reading_form(text)
    ...
    text = cyrrilize(text)
    text = delete_spec_symbols(text)
    text = replace_multiples_spaces(text)
    text = add_dot_to_end(text)
    return text