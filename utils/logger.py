...
class Tacotron2Logger(SummaryWriter):
    ...
    def sample_infer(self, outputs, iteration, idx):
            mel_outputs = to_arr(outputs[0][0])
            mel_outputs_postnet = to_arr(outputs[1][0])
            ...
            self.add_audio(f'infer.wav {idx}', wav, iteration, hps.sample_rate)
            self.add_audio(f'infer.wav_post {idx}', wav_postnet, iteration, hps.sample_rate)
