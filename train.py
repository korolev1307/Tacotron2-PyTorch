...
def train(args):
    if 'WORLD_SIZE' in os.environ:
        ...
        for key in ['WORLD_SIZE', 'OMP_NUM_THREADS', 'RANK', 'LOCAL_RANK']:
            print(f'{key}: {os.environ[key]}')
    else:
        n_gpu = 1
        rank = local_rank = 0
    if n_gpu > 1:
        print(f'{n_gpu} init started')
        torch.distributed.init_process_group(backend = 'nccl', 
                                             init_method = 'env://', 
                                             rank = rank, 
                                             world_size = n_gpu)
    print(f'{n_gpu} GPUS inited')
    ...
    while iteration <= hps.max_iter:
        ...
        for batch in train_loader:
            ...
            if rank == 1:
                # info
                print('RANK[1] Iter: {} Mel Loss: {:.2e} Gate Loss: {:.2e} Grad Norm: {:.2e} {:.1f}s/it'.format(
                    iteration, items[0], items[1], grad_norm, dur))
            if rank == 0:
                # info
                print('RANK[0] Iter: {} Mel Loss: {:.2e} Gate Loss: {:.2e} Grad Norm: {:.2e} {:.1f}s/it'.format(
                    iteration, items[0], items[1], grad_norm, dur))
                ...
                if args.log_dir != '' and (iteration % hps.iters_per_sample == 0):
                    ...
                    for idx, infer_value in enumerate(hps.eg_text):
                        output = infer(infer_value, model.module if n_gpu > 1 else model)
                        logger.sample_infer(output, iteration, idx) 
                    model.train()
